<script lang="ts">
	...
	function handleVersionSwap(
		mod: Mod,
		newVersion: { loader: string; version: string; jarName: string }
	) {
		const modIndex = allMods.findIndex((m) => m.name === mod.name);
		if (modIndex !== -1) {
			allMods[modIndex] = {
				...allMods[modIndex],
				loader: newVersion.loader,
				version: newVersion.version,
				jarName: newVersion.jarName
			};
		}
	}

	function handleVersionSelect(version: AvailableVersion) {
		if (selectedMod) {
			handleVersionSwap(selectedMod, version);
			handleCloseDrawer();
		}
	}
</script>
...
<div class="flex h-full flex-col overflow-hidden">
	...
	>
		...
		<div class="ml-2 flex items-center gap-2">
			<span class="text-sm font-medium whitespace-nowrap" style="color: var(--text-sec);"
				>Loader</span
			>
			<div
				class="flex h-8 overflow-hidden rounded-sm border"
		...
		<div class="ml-2 flex flex-1 items-center gap-2">
			...
			<div class="relative flex flex-1">
				<input
					...
					class="h-8 w-full rounded-l-sm border px-3 text-sm transition-colors focus:ring-0 focus:outline-none"
					style="
				...
				<button
					...
					class="flex h-8 items-center rounded-r-sm border-y border-r px-3 transition-colors hover:brightness-110"
					style="
					background-color: var(--bg-input);
					border-color: var(--border-color);
					color: var(--text-sec);"
		...
		<button
			...
			class="flex h-8 items-center gap-2 rounded-sm px-6 text-sm font-medium text-white shadow-lg transition-colors hover:brightness-110"
				...
				<div class="flex gap-3">
					<div class="relative">
						...
						<input
							...
							class="h-9 w-64 rounded-sm border pr-3 pl-8 text-sm transition-colors outline-none focus:ring-0 focus:outline-none"
							style="
								background-color: var(--bg-input);
								border-color: var(--border-color);
								color: var(--text-main);"
